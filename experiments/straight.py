...
if __name__ == '__main__':
    ...
    kappa = 0.00001 # 1/20.0
    k1 = 0 #0.05 #imaginary value
    x0 = vector(0,0)
    ...
    racer = Skier(mi, alfa1, k1, k2, B, m1, x0, v0, kappa, slalom,solver, kappa_controller_A, color="blue")
    racer2 = Skier(mi, alfa1, k1, k2, B, m2, x0, v0, kappa, slalom, solver, kappa_controller_B, color="green")
    racer3 = Skier(mi, alfa2, k1, k2, B, m1, x0, v0, kappa, slalom,solver, kappa_controller_A, color="red")
    racer4 = Skier(mi, alfa2, k1, k2, B, m2, x0, v0, kappa, slalom, solver, kappa_controller_B, color="cyan")

    sim = SkierSimulation(distance=200, interval=0.01, time_zoom=100)
    sim.add_racer(racer)
    sim.add_racer(racer2)
    sim.add_racer(racer3)
    sim.add_racer(racer4)
    sim.run()
    ...
    pylab.grid(True)
    pylab.show()
    pylab.plot(sim.timeline,[ y for [x,y,z] in racer.positions], 
           sim.timeline,[ y for [x,y,z] in racer2.positions],
           sim.timeline,[ y for [x,y,z] in racer3.positions],
            sim.timeline,[ y for [x,y,z] in racer4.positions],)
    
    pylab.legend(('narciarz 60 kg, stromy stok', 'narciarz 100 kg, stromy stok', 'narciarz 60 kg, lagodny stok','narciarz 100 kg, lagodny stok'), loc='lower right')
    pylab.xlabel("czas w sekundach")
    pylab.ylabel("dystans w metrach")
    pylab.grid(True)
    pylab.show()